...
- <details><summary>산출물</summary>
  ...
  14. WBS 및 산출물 관리
      ...
      - H*****t_APPVERSION_API_Reference.pdf
      - RENTA2_DDL.sql
  ...
  23. 담당자별 인터뷰 시 보여준 자료 모음
      - ★ 2021 렌터카사업팀 사업계획 명의변경 프로세스 개선 (210422)_이**매니저(11월 9일자 인터뷰).pptx_이**매니저(11월_9일자_인터뷰).pptx)
      - 반납 프로세스Ⅲ.pptx
  ...
  24. 요구사항 ID 정의서
      - KN렌터카_요구사항정의서_v1.4.0_211206.xlsx
  25. WBS
      - KN렌터카_WBS_v1.1.11_220214_휴**업데이트.xlsx
  26. 연동 API 관련 문서
      ...
      - KN차_렌터카시스템_연동_인터페이스명세서_V1.00_보안해제 (2).xls
      - C**PApi (1).zip
      - 통합계정 API (1).pdf
  ...
  27. 대고객 화면 설계서
      ...
      - KN렌터카_화면설계서_대고객서비스_v1.0.3_220322.pptx
  ...
  42. KN렌터카_버그리포트
      - KN렌터카_버그리포트(취합용)_221026.xlsx_221026.xlsx)
  ```
...
> Q) 대고객 화면 그리는 로직에도 문제가 있어 보임, LazyLoading, 단위 끊어 그리기, ... 화면단 해결 방법도 있어보였음. <br>
> A) 맞음, 근데 페이징이 근본적으로 필요했어서 그 방법을 채택했음

<br>

> Q) Query 관련 DB 공부 부분이 제일 중요하지 않나, <br> 
> A) ㅇㅈ

<br>

> Q) 시간 관련 테스트 코드를 하는게 좋다고 했는데, 시간 검증 값은 얼마를 잡아야 하는지? <br>
> A) 나도 잘 모름, 절대적 지표는 없을 것 같다고 생각함. 프로파일링, 튜닝 관련 책들에서 살펴보는것이 좋아 보임

<br>

> Q) 비동기식 API에 비해, 동기식 API 등에는 무한한 관심이 필요 <br> 동기처리 되어야 하는 API들은 최종 성능에 문제로 보일 수 있는 최상단 부분 <br> 
> Q) 배치 관련 이나 대용량성 API 응답, 소비처를 고려해서 Queue, Batch, 등의 다른 요소들도 고려해보아야함 <br>
> A) ㅇㅈ
...
> Q) 의도 전달 마지막 회의록 부분이나 접근 방식이 다른 사람에게 불쾌할 수도 있음, <br> 대부분의 의도 전달은 송신자에게 역할 및 책임이 큼 <br> 
> 
> Q) 회의, 비즈니스 같은 회의에서 다른 직무 영역, 개발자, 기획자, QA 한 측 입장에서 시작하는 회의 등 <Br> 모임의 성질을 잘 파악하고 참여할 필요가 있음 
...
_기간 산출 부터 잘못, 기획 내용이 잘못, 기획 내용이 수정, 개발자가 기간내에 끝낼 수 있는 량을 못 끝냄, 시발 휴** 솔루션 쓴다매 개새끼들아_
...
> Q) 개발 기간 예측 영역이 사실 애초에 어려운 영역임, 목표를 세우는게 말이 안되는데 세워야함 <br>
> Q) 실패가 당연하다고 생각함,, 단  실패 사유를 고려하는 회고는 중요하다고 생각함. <br>
> Q) 실패에는 여러가지 이유가 있을 수 있음,
> 
> <br>
> Q) 최선, 최고, 최적 관련은 당연한 의견인데, 우리가 항상 최적을 고려할 수 없다. <br>
> Q) 때로는 최악이 최적일 수 있다.(미래를 봤을 때) <br>
> A) 최악의 선택도 장기적으로 최적이 될 수 있다는 부분이 와닿았음. <br>
> A) 선택 근거가 늘어 날 수록 최악이나 최적이 뒤바 뀔수 있다가 결론이지 않을까?
...
_[Tiki 바로 가기](https://www.notion.so/H*****t/Tiki-cb8bbddf1cbb4289b4141208f3d22bb0)_


`Tiki`는 개발지식 공유 `Wiki`임

_휴** 노션 개발 가이드 최하단에 있음_
...
> Q) TikiTaka 축구에서 그거아닌데 ㅋ <br>
> Q) 뭔가 축구에서는 삼각형 대형을 이루고 서로의 대형을 유지하고 하는게 핵심이고,
> <br> 해당 발표에서도 서로간의 개발 합을 중요시 하는게 전제인데, <br>
> 달려나가는 쪽? 에 치우친 방향이지 않나? 서로를 살피고 대형을 맞추는게 중요한 TikiTaka 라고 생각함
> 
> A) 헉 , 다음 피트스탑에 관련 내용 있음.
