...
**내가 그 당시 느꼇던 내 생각을 공유**하며, **다른이들과 의견을 공유**하고 **다음으로 나아가기 위함**임.
...
- 개발자 마다 다른 쿼리 작성
  - 초기 테이블도 추후에 갈아엎을 상황에 각각 다른 테이블 이해도를 가지며 비슷하지만 각기 다른 쿼리 작성
...
이 `뚱뚱이` 생성 작업은 뇌지컬도 필요없는 단순한 반복 작업이었음.
...
지난 19개월 간 회사에 여러가지를 도입해보려고 노력했다.
...
지난 13개월 간 수행한 `KN` Project에서 여러가지 개발방식을 도입 및 전파해보려고 노력했다.
...
위 목록들은 세 가지 상태로 분류되었다.

_**실패했거나, 도입에 바로 성공하거나, 뒤늦게라도 도입되거나**_

<br>

**위 세 가지 분류의 차이점을 생각하다가 든 생각**은,

**한 가지 공통 키워드가 존재**하는 것이었다.

<br>

사실 저 모든 목록은 **필요성과 관련**이 있다.

<br>

**도입에 실패한 항목**은 **필요성을 납득 시키는 것이 현재까지도 실패한 상태**,

**도입에 바로 성공한 항목**은 **이미 그 필요성을 인지하고 있던 상태**,

**뒤늦게 도입된 항목**은 **뒤늦게 필요성을 인지한 상태**로 구분지을 수 있다.
...
**구성원이 납득하고 필요하다고 느껴지는것들**은 도입이 되면 `제도`가 된다.

**구성원이 납득하지 못하고 필요하다고 느끼지 못하는것들**은 도입이 되더라도 `관습`이 된다.
...
**나는 지난 19개월간 아래와 같은 노력**만 했다.

>`여러가지` `유용할 것들`의 도입을 시도 했는데,

<br>

**`여러가지`, `유용할 것들`에 치중**하느라, 우리가 **`필요한것들`을 놓쳐왔다.**

**올바른 제도 도입은 아래 문장**과 같다.

> 우리가 필요한 것들의 도입을 시도한다.

<br>

나는 앞으로는 우리가 `필요한 것들`에 집중 해보려 한다.

나는 앞으로는 관습이 아닌 제도를 도입해보려 한다.
